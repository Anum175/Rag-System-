...
if "chats" not in st.session_state:
    st.session_state.chats = {"General": []}
...
if st.sidebar.button("Add New Chat"):
    if new_chat_name and new_chat_name not in st.session_state.chats:
        ...
        st.session_state.new_chat_name = ""  # Reset input
        st.session_state.edit_mode = {}  # Clear edit mode
    ...
    if col3.button("🗑️", key=f"delete_{chat_name}"):
        if chat_name != "General":
            ...
            if st.session_state.current_chat == chat_name:
                st.session_state.current_chat = "General"
            st.session_state.edit_mode.pop(chat_name, None)  # Remove edit mode entry safely
            st.session_state.new_chat_name = ""
    ...
    if st.session_state.edit_mode.get(chat_name, False):
        ...
        if st.sidebar.button("Save", key=f"save_{chat_name}"):
            if new_name and new_name != chat_name:
                ...
                st.session_state.edit_mode.pop(chat_name, None)  # Remove old edit mode entry safely
                st.session_state.new_chat_name = ""
                st.experimental_rerun()
...
if st.button("Submit Query"):
    if question:
        ...
        st.session_state.chats[st.session_state.current_chat].append(f"You: {question}")
        st.session_state.chats[st.session_state.current_chat].append(f"Bot: {response}")
